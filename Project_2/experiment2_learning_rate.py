...
for alpha in alphas:
    ...
    for episode in range(episodes):
        score=0
        state = env.reset()
        print('eps {} alp {}'.format(episode, alpha))
...
plt.figure()
plt.plot(range(len(scores)), final_scores[alphas[0]])
plt.plot(range(len(scores)), final_scores[alphas[1]])
plt.plot(range(len(scores)), final_scores[alphas[2]])
plt.legend(["alpha 5e-2", "alpha 5e-3", "alpha 5e-4"])
plt.xlabel("episode")
plt.ylabel("score")