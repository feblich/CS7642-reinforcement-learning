...
for epsilon_decay in epsilon_decays:
    ...
    for episode in range(episodes):
        score=0
        state = env.reset()
        print('eps {} epsd {}'.format(episode, epsilon_decay))
    ...
    final_scores[epsilon_decay] = scores



plt.figure()
plt.plot(range(len(scores)), final_scores[epsilon_decays[0]])
plt.plot(range(len(scores)), final_scores[epsilon_decays[1]])
plt.plot(range(len(scores)), final_scores[epsilon_decays[2]])
plt.legend(["epsilon_decays 1", "epsilon_decays .995", "epsilon_decays .9"])
plt.xlabel("episode")
plt.ylabel("score")